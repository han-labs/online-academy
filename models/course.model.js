    ...
    async findByInstructor(instructorId) {
        ...
        const coursesWithStatus = await Promise.all(
            courses.map(async (course) => {
                const [{ total_lessons }] = await db('lectures')
                    .join('chapters', 'chapters.id', 'lectures.chapter_id')
                    .where('chapters.course_id', course.id)
                    .count({ total_lessons: '*' });

                const [{ completed_lessons }] = await db('lectures')
                    .join('chapters', 'chapters.id', 'lectures.chapter_id')
                    .leftJoin('lecture_progress', function () {
                        this.on('lectures.id', '=', 'lecture_progress.lecture_id')
                            .andOn('lecture_progress.user_id', '=', db.raw('?', [instructorId]));
                    })
                    .where('chapters.course_id', course.id)
                    .count({ completed_lessons: '*' });

                const is_completed = total_lessons > 0 && total_lessons == completed_lessons;
